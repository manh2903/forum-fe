  ...
  return (
    <Box>
      ...
      <Paper sx={{ p: 3, borderRadius: 3, border: "1px solid #e2e8f0", mb: 3, position: "relative" }}>
        ...
        {profile.role !== "user" && <Chip label={profile.role} color="primary" size="small" sx={{ mb: 1 }} />}
        {profile.bio && (
      ...
      <Box>
        ...
        {tab === 2 ? (
          reportsLoading ? (
            ...
            <Stack spacing={2}>
              {reports.map((report: any) => (
                <Paper key={report.id} sx={{ p: 2, borderRadius: 2, border: "1px solid #e2e8f0" }}>
                  <Box sx={{ display: "flex", justifyContent: "space-between", mb: 1.5 }}>
                    <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                      <ReportIcon sx={{ color: "error.light", fontSize: 20 }} />
                      <Typography variant="subtitle2" fontWeight={700}>
                        Báo cáo {report.targetType === "post" ? "bài viết" : report.targetType === "comment" ? "bình luận" : "người dùng"}
                      </Typography>
                    </Box>
                    <Chip
                  ...
                  <Typography variant="body2" sx={{ mb: 1, color: "text.primary" }}>
                    <strong>Lý do:</strong> {REASON_LABELS[report.reason]}
                  </Typography>
                  {report.description && (
                    <Typography variant="body2" sx={{ mb: 1, color: "text.secondary", bgcolor: "#f8fafc", p: 1, borderRadius: 1 }}>
                      {report.description}
                    </Typography>
                  )}
                  {report.target && (
                    <Box sx={{ pl: 2, borderLeft: "2px solid #e2e8f0", mb: 1 }}>
                      ...
                      {report.targetType === "post" ? (
                        ...
                        <Typography variant="body2" sx={{ fontWeight: 600 }}>
                          Người dùng: @{report.target.username}
                        </Typography>
                      )}
                    </Box>
                  )}
                  {report.status !== "pending" && report.resolution && (
                    <Box sx={{ mt: 1.5, p: 1.5, bgcolor: alpha("#10b981", 0.05), borderRadius: 1.5, border: "1px dashed #10b981" }}>
                  ...
                  <Typography variant="caption" color="text.secondary" sx={{ mt: 1, display: "block" }}>
                    Gửi lúc: {dayjs(report.createdAt).format("DD/MM/YYYY HH:mm")}
                  </Typography>
                </Paper>
              ))}
              {reports.length === 0 && (
          ...
          <Stack spacing={2}>
            {posts.map((post) => (
              <PostCard key={post.id} post={post} showStatus={isOwnProfile && tab === 0} />
            ))}
            {posts.length === 0 && (
              <Typography align="center" color="text.secondary" sx={{ py: 4 }}>
      ...
      <Dialog open={userListDialog.open} onClose={() => setUserListDialog((p) => ({ ...p, open: false }))} fullWidth maxWidth="xs">
        <DialogTitle sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <IconButton size="small" onClick={() => setUserListDialog((p) => ({ ...p, open: false }))}>
            <CloseIcon />
          </IconButton>
        </DialogTitle>
      ...
      {profile && <ReportDialog open={reportOpen} onClose={() => setReportOpen(false)} targetType="user" targetId={profile.id} />}
    </Box>
  );
}
